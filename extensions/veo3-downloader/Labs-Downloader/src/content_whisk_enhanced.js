...
(function() {
    ...
    function makePanelDraggable() {
        ...
        if (!container || !header) {
            whiskLog.error('Container ou header não encontrado para drag');
            return;
        }
        ...
        function dragStart(e) {
            // Não arrasta se clicar no botão minimizar ou toggle
            if (e.target.closest('.whisk-minimize-btn') ||
                e.target.closest('.whisk-toggle-switch') ||
            ...
            // Pega a posição atual do painel
            const rect = container.getBoundingClientRect();

            // Calcula o offset entre o clique e o canto do painel
            ...
            container.style.transition = 'none'; // Remove transição durante drag

            e.preventDefault();
            whiskLog.info('Drag iniciado');
        }
        ...
        function dragEnd(e) {
            ...
            localStorage.setItem('whisk-panel-position', JSON.stringify({
                x: finalX,
                y: finalY
            }));

            whiskLog.info(`Drag finalizado - Nova posição: ${finalX}, ${finalY}`);
        }
