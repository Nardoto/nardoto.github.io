// sidebar-script.js – Prompt Manager 5.0 - Versão Final Completa com Melhorias
...
function initializePromptManager() {
    ...
    console.log("✅ Prompt Manager 5.0 inicializado");
}
...
function changeChannel(e) {
    ...
    // Se selecionou o modo bulk sender
    if (selectedValue === '__BULK_SENDER__') {
        state.currentChannel = '__BULK_SENDER__';
        toggleBulkSenderMode(true);
        return;
    }

    // Modo normal - voltar do bulk sender se necessário
    toggleBulkSenderMode(false);

    state.currentChannel = selectedValue;
    state.currentCategory = Object.keys(channels[state.currentChannel].categories)[0];
    syncStateVariables();
    updateUI();
...
function sendPrompt(p) {
    ...
    // Marcar prompt como enviado sempre que o botão Enviar for apertado
    usedPrompts.add(p.id);
    updatePromptItemStatus(p.id, 'used');
...
function updateChannelSelect() {
    ...
    // Adiciona divisor e opção especial de Envio em Massa
    ...
    bulkOption.value = '__BULK_SENDER__';
    bulkOption.textContent = '📤 Envio em Massa';
    if (state.currentChannel === '__BULK_SENDER__') bulkOption.selected = true;
    sel.appendChild(bulkOption);
}
...
function updatePromptItemStatus(promptId, status) {
    ...
    promptElement.classList.remove('sequence-current', 'sequence-completed', 'sequence-error', 'sequence-pending', 'sequence-used');
    ...
    if (status) {
        ...
        switch (status) {
            case 'current':
                ...
                console.log(`🟠 Prompt ${promptId} marcado como ATUAL (laranja com pulse)`);
                break;
            case 'completed':
            ...
            case 'used':
                ...
                statusMsg.style.color = '#10b981';
                console.log(`🟢 Prompt ${promptId} marcado como ENVIADO (verde claro)`);
                break;
        }
    } else {
...
let bulkPromptsQueue = [];
let currentBulkIndex = 0;
let bulkResults = []; // Array para armazenar respostas do bulk sender

/**
 * Toggle between normal mode and bulk sender mode
 */
function toggleBulkSenderMode(showBulkSender) {
    const bulkContainer = document.getElementById('bulkSenderContainer');
    const promptsList = document.getElementById('promptsList');
    const categorySection = document.querySelector('.category-section');
    const newPromptContainer = document.querySelector('.new-prompt-container');
    const searchSection = document.querySelector('.compact-search');
    const sequenceConfig = document.querySelector('.sequence-config');
    const channelActions = document.querySelector('.channel-actions');

    if (showBulkSender) {
        // Mostrar bulk sender, esconder elementos normais
        if (bulkContainer) bulkContainer.classList.remove('hidden');
        if (promptsList) promptsList.style.setProperty('display', 'none', 'important');
        if (categorySection) categorySection.style.setProperty('display', 'none', 'important');
        if (newPromptContainer) newPromptContainer.style.setProperty('display', 'none', 'important');
        if (searchSection) searchSection.style.setProperty('display', 'none', 'important');
        if (sequenceConfig) sequenceConfig.style.setProperty('display', 'none', 'important'); // Oculta Auto Play Mode
        if (channelActions) channelActions.style.setProperty('display', 'none', 'important'); // Oculta botões de ação do canal

        console.log('📤 Modo Envio em Massa ativado');
    } else {
        // Mostrar elementos normais, esconder bulk sender
        if (bulkContainer) bulkContainer.classList.add('hidden');
        if (promptsList) promptsList.style.display = '';
        if (categorySection) categorySection.style.display = '';
        if (newPromptContainer) newPromptContainer.style.display = '';
        if (searchSection) searchSection.style.display = '';
        if (sequenceConfig) sequenceConfig.style.display = ''; // Mostra Auto Play Mode
        if (channelActions) channelActions.style.display = ''; // Mostra botões de ação do canal
...
/**
 * Parse prompts from textarea - split by blank lines
 */
function parseBulkPrompts(text) {
    if (!text || !text.trim()) return [];

    // Split by double line breaks (blank lines)
...
function updateBulkPromptCount() {
    const textarea = document.getElementById('bulkPromptsInput');
    const countSpan = document.getElementById('bulkPromptCount');
    const startBtn = document.getElementById('startBulkSendBtn');

    if (!textarea || !countSpan || !startBtn) return;

    const prompts = parseBulkPrompts(textarea.value);
    const count = prompts.length;

    countSpan.textContent = `${count} prompt${count !== 1 ? 's' : ''} detectado${count !== 1 ? 's' : ''}`;
    startBtn.disabled = count === 0 || isBulkSending;
}
...
function updateBulkProgress(current, total, status = '') {
    ...
    if (progressText) progressText.textContent = status || 'Enviando...';
    if (progressCount) progressCount.textContent = `${current}/${total}`;
    if (progressFill) {
...
/**
 * Send single bulk prompt and wait for response
 */
async function sendSingleBulkPrompt(promptData) {
    console.log(`📤 Enviando prompt ${currentBulkIndex + 1}/${bulkPromptsQueue.length}: "${promptData.title}"`);

    return new Promise((resolve, reject) => {
        // Listener para capturar a resposta
        const responseHandler = (event) => {
            document.removeEventListener('promptmanager-sequence-response', responseHandler);

            const { success, copiedText } = event.detail || {};

            console.log(`📨 Resposta recebida para prompt ${currentBulkIndex + 1}:`, {
                success,
                textLength: copiedText?.length || 0
            });
            ...
            bulkResults.push({
                ...
                status: success && copiedText ? 'completed' : 'error',
                responseText: copiedText || 'ERRO: Resposta vazia ou não capturada',
                timestamp: new Date().toLocaleString()
            });
            ...
            if (success && copiedText) {
                console.log(`✅ Prompt ${currentBulkIndex + 1} enviado e resposta capturada (${copiedText.length} chars)`);
                resolve();
            } else {
                console.warn(`⚠️ Prompt ${currentBulkIndex + 1} enviado mas resposta não capturada adequadamente`);
                resolve(); // Continua mesmo sem resposta
            }
        };

        // Adiciona listener temporário
        document.addEventListener('promptmanager-sequence-response', responseHandler);

        // Timeout de segurança (3 minutos)
        const timeoutId = setTimeout(() => {
            document.removeEventListener('promptmanager-sequence-response', responseHandler);
            console.warn(`⏱️ Timeout ao aguardar resposta do prompt ${currentBulkIndex + 1}`);

            // Adiciona resultado de timeout
            bulkResults.push({
                promptTitle: promptData.title,
                promptContent: promptData.content,
                status: 'timeout',
                responseText: 'ERRO: Timeout - Resposta não capturada em 3 minutos',
                timestamp: new Date().toLocaleString()
            });
            updateBulkPreview();
        ...
        try {
            // Dispatch event to content script usando o evento de sequência
            const event = new CustomEvent('promptmanager-sequence-send', {
                detail: {
                    text: promptData.content,
                    totalCount: bulkPromptsQueue.length,
                    waitTime: 30000
                }
            });
            document.dispatchEvent(event);

            console.log(`📨 Evento de envio disparado para prompt ${currentBulkIndex + 1}`);
        } catch (error) {
            clearTimeout(timeoutId);
            document.removeEventListener('promptmanager-sequence-response', responseHandler);
            console.error(`❌ Erro ao enviar prompt ${currentBulkIndex + 1}:`, error);
            reject(error);
        }
    });
}
...
/**
 * Update bulk preview with all responses
 */
...
/**
 * Start bulk sending sequence
 */
async function startBulkSend() {
    const textarea = document.getElementById('bulkPromptsInput');
    if (!textarea || !textarea.value.trim()) {
        showStatus('Por favor, insira alguns prompts primeiro.', 'error');
        return;
    }

    bulkPromptsQueue = parseBulkPrompts(textarea.value);

    if (bulkPromptsQueue.length === 0) {
        showStatus('Nenhum prompt detectado. Separe os prompts com linhas em branco.', 'error');
        return;
    }

    console.log(`🚀 Iniciando envio em lote de ${bulkPromptsQueue.length} prompts`);

    isBulkSending = true;
    currentBulkIndex = 0;
    bulkResults = []; // Limpa resultados anteriores

    // Update UI
    document.getElementById('startBulkSendBtn').style.display = 'none';
    document.getElementById('stopBulkSendBtn').style.display = 'block';
    document.getElementById('bulkPromptsInput').disabled = true;

    showStatus(`Enviando ${bulkPromptsQueue.length} prompts em sequência...`, 'info');

    try {
        for (let i = 0; i < bulkPromptsQueue.length; i++) {
            if (!isBulkSending) {
                console.log('⏹ Envio em lote interrompido pelo usuário');
                break;
            }

            currentBulkIndex = i;
            const promptData = bulkPromptsQueue[i];

            updateBulkProgress(i + 1, bulkPromptsQueue.length, `Enviando: ${promptData.title}`);

            await sendSingleBulkPrompt(promptData);

            // Wait between prompts (except for the last one)
            // Tempo padrão: 30 segundos (mesmo do modo sequência automática)
            if (i < bulkPromptsQueue.length - 1 && isBulkSending) {
                const waitTime = 30000; // 30 segundos
                console.log(`⏳ Aguardando ${waitTime / 1000} segundos para Claude processar antes do próximo prompt...`);
                updateBulkProgress(i + 1, bulkPromptsQueue.length, `Aguardando próximo prompt... (${waitTime / 1000}s)`);
                await new Promise(resolve => setTimeout(resolve, waitTime));
            }
        }

        if (isBulkSending) {
            console.log('✅ Todos os prompts foram enviados com sucesso!');
            showStatus(`${bulkPromptsQueue.length} prompts enviados com sucesso!`, 'success');
            updateBulkProgress(bulkPromptsQueue.length, bulkPromptsQueue.length, 'Concluído!');
        } else {
            showStatus('Envio em lote interrompido.', 'warning');
        }
    } catch (error) {
        console.error('❌ Erro durante envio em lote:', error);
        showStatus('Erro durante o envio em lote.', 'error');
    } finally {
        stopBulkSend();
    }
}
...
function stopBulkSend() {
    console.log('⏹ Parando envio em lote...');
    isBulkSending = false;

    // Reset UI
    document.getElementById('startBulkSendBtn').style.display = 'block';
    document.getElementById('stopBulkSendBtn').style.display = 'none';
    document.getElementById('bulkPromptsInput').disabled = false;
    updateBulkPromptCount();

    // Hide progress after a delay
    setTimeout(() => {
        const progressDiv = document.getElementById('bulkProgress');
        if (progressDiv && !isBulkSending) {
            progressDiv.style.display = 'none';
        }
    }, 3000);
}

// Initialize bulk sender event listeners
(function initBulkSender() {
    // Update count when textarea changes
    const bulkTextarea = document.getElementById('bulkPromptsInput');
    if (bulkTextarea) {
        bulkTextarea.addEventListener('input', updateBulkPromptCount);
    }

    // Start button
    const startBtn = document.getElementById('startBulkSendBtn');
    if (startBtn) {
        startBtn.addEventListener('click', startBulkSend);
    }

    // Stop button
    const stopBtn = document.getElementById('stopBulkSendBtn');
    if (stopBtn) {
        stopBtn.addEventListener('click', stopBulkSend);
    }

    console.log('✅ Bulk Sender inicializado');
})();