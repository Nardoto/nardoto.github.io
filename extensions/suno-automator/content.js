...
async function populateSunoAndCreate(songData) {
    try {
        ...
        // Seletores atualizados baseados na estrutura real do Suno (2025)
        ...
        // Seletor atualizado para o campo de styles (indie, electronic, synths, 120bpm, distorted)
        const styleField = document.querySelector('textarea[placeholder*="indie, electronic, synths"]') ||
                         document.querySelector('textarea[placeholder*="indie"]') ||
                         document.querySelector('textarea[placeholder*="synths"]') ||
                         document.querySelector('textarea.resize-none[maxlength="1000"]') ||
                         document.querySelector('textarea[placeholder*="bpm"]');

        // Seletor atualizado para o campo de título (Song Title (Optional))
        // Nota: Este input não tem type="text" explícito
        const titleField = document.querySelector('input[placeholder*="Song Title (Optional)"]') ||
                         document.querySelector('input[placeholder*="Song Title"]') ||
                         document.querySelector('input.ml-2.flex-1.bg-transparent.py-2') ||
                         document.querySelector('input.ml-2.flex-1[placeholder]');

        const createButton = Array.from(document.querySelectorAll('button')).find(btn =>
            btn.textContent.trim().toLowerCase() === 'create' ||
        ...
        console.log("🔍 Seletores usados:", {
            lyricsSelector: lyricsField?.getAttribute('placeholder') || lyricsField?.getAttribute('name'),
            styleSelector: styleField?.getAttribute('placeholder') || styleField?.getAttribute('name'),
            titleSelector: titleField?.getAttribute('placeholder') || titleField?.getAttribute('name')
        });
        
        if (lyricsField) {
            await fillField(lyricsField, songData.lyrics);
            console.log("✅ Lyrics field preenchido");
        } else {
            throw new Error("Lyrics field not found");
        }
        
        if (styleField) {
            await fillField(styleField, songData.style);
            console.log("✅ Style field preenchido");
        } else {
            throw new Error("Style field not found");
        }
        
        if (titleField) {
            await fillField(titleField, songData.title);
            console.log("✅ Title field preenchido");
        } else {
            console.error("❌ Title field not found com seletores principais");
            console.log("🔍 Tentando fallback: buscando todos os inputs visíveis...");

            // Fallback: busca qualquer input visível
            const allInputs = Array.from(document.querySelectorAll('input'));
            const visibleInputs = allInputs.filter(input => input.offsetParent !== null);

            console.log(`📊 Total: ${allInputs.length} inputs, ${visibleInputs.length} visíveis`);

            // Procura por input com "title" no placeholder
            const fallbackTitle = visibleInputs.find(input =>
                input.placeholder?.toLowerCase().includes('title')
            );

            if (fallbackTitle) {
                console.log(`✅ Fallback encontrou: placeholder="${fallbackTitle.placeholder}"`);
                await fillField(fallbackTitle, songData.title);
                console.log("✅ Title field preenchido (via fallback)");
            } else {
                throw new Error("Title field not found - verifique se está na página /create");
            }
        }
        ...
        if (createButton) {
            console.log("🎵 Clicando no botão Create...");
            createButton.click();
...
async function fillField(element, value) {
    ...
    // Aguarda um pouco para garantir que o foco foi aplicado
